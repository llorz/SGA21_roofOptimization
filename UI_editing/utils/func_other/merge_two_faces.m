function obj_new = merge_two_faces(obj, ref_fid1_in, ref_fid2_in)
ref_fid1 = min(ref_fid1_in, ref_fid2_in);
ref_fid2 = max(ref_fid1_in, ref_fid2_in);
...
% project all the outline vtx at the same line.
eid = [eid1, eid2];
eid = eid(1);
vids = unique([obj.F{ref_fid1}; obj.F{ref_fid2}]);
ovids = vids(ismember(vids, obj.vid_outline));
t = obj.compute_edge_direction(eid);
x0 = obj.V(obj.E(eid,1),:);
