function [X1, X_ini] = construct_3D_roof_pavilion(V_all, F, vid_xy, vid_z, vid_z_var, lambda, X_ini,lambda2)
if nargin < 8
    lambda2= 1;
end
% initial position
x0 = [reshape(X_ini(vid_xy,1:2),[],1); X_ini(vid_z,3)];
func = @(x) energy_roof_graph(x, vid_xy, vid_z, vid_z_var, X_ini, F, V_all, lambda, lambda2);
...
function fval = energy_roof_graph(x, vid_xy, vid_z, vid_z_var, X_ini, F, X_input, lambda,lambda2)
...
for ii = 1:length(vid_z_var)
    fval = fval+ lambda2*var(X(vid_z_var{ii},3));
end
end
